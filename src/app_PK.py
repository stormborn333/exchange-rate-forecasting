...
import json
# import datetime - usunąłem dubluje się !!!!!!!!!!!!!!!!

#getting tickers data
with open("data-utils/data-raw/tickers.json", 'r') as f:
        data_tickers = json.load(f)
ticker_names = list(data_tickers.values())[11:]# nazwy tickerów, zaczynając od 12-tego elementu, dane - predykcje !!!!!!!!!!!!!
...
app.layout = html.Div([
    ...
    dcc.DatePickerRange(
        ...
        end_date = date.today().strftime("%Y-%m-%d"), # data od dzisiaj !!!!!!!!!!!!!!!
        display_format='YYYY-MM-DD',
        min_date_allowed = "2008-01-01",
        max_date_allowed = date.today().strftime("%Y-%m-%d") # data od dzisiaj !!!!!!!!!!!!!!!
        ),
...
def update_graph(show_clicks, reset_clicks, predict_days, selected_ticker, start_date, end_date):
    ...
    if not ctx.triggered:
         return (dash.no_update, {'display': 'none'}, dash.no_update, {'display': 'none'}, 
                 ...
                 date.today().strftime("%Y-%m-%d")) # default values after reset  # data od dzisiaj !!!!!!!!!!!!!!!
    ...
    if button_id == 'generate-results-button':
        ...
        df_forecast = df_forecast [-100:] # zmiana zakresu wyświetlania ostatnie 100 dni !!!!!!!!!!!!!!!
        ...
        #result table
        df_results = pd.DataFrame({'Date': future_dates.strftime('%Y-%m-%d'), # data od dzisiaj !!!!!!!!!!!!!!!
    ...
    return (dash.no_update, {'display': 'none'},dash.no_update, {'display': 'none'},dash.no_update, {'display': 'none'}, dash.no_update, 
            {'display': 'none'},dash.no_update, {'display': 'none'}, [],[], 5, 'S&P 500', "2008-01-01", date.today().strftime("%Y-%m-%d")) # data od dzisiaj !!!!!!!!!!!!!!!
