...
def analyze_ticker(start, end, ticker, num_days):
    ...
    # pobranie danych ze strony Yahoo
    ...
    plt.plot(data.index, data["Close"], linestyle='-', color='darkblue')
    plt.xlabel('Lata')
    plt.ylabel('Wartość [USD]')
    plt.title(f'Wartość zamknięcia dla {tickers[ticker]}')
    plt.grid(color='lightgray')
    plt.tight_layout()
    plt.savefig("zamkniecie.jpg")
    plt.close()
